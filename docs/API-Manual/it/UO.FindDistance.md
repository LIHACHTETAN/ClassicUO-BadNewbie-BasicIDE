...
Legge o modifica il raggio orizzontale predefinito della ricerca.
...
- `value` — Integer facoltativo. Senza argomento legge; value imposta. Limitato a 0..255; un negativo diventa 0, non illimitato. Nuovo runtime: 18; lo stato ripristinato può differire. I decimali sono troncati verso zero; sono ammesse stringhe numeriche decimal/0x. Usare Integer evita conversioni implicite.

## Restituisce

Senza argomenti: Integer, limite corrente (distanza in caselle), non ID, conteggio o Boolean. 0 indica limite zero, non fallimento. Con value: Unit, nessun valore, né TRUE/FALSE né valore precedente. Rileggere FindDistance() dopo la modifica.

## Comportamento

- Distanza: max(abs(dx), abs(dy)) dall’origine corrente della portata del client; una diagonale vale una casella. Limite incluso; 0 accetta solo le stesse X/Y. Per Mobile in movimento conta la posizione finale della coda dei passi.
...
- Riferimento: [Stealth FindDistance](https://stealth.od.ua/api/FindDistance/). Questo client conserva valori iniziali/intervalli propri: FindDistance 18 / 0..255; FindVertical 2 / 0..120. Sintassi Basic e filtri estesi descrivono questo progetto.
...
Sorgente del progetto: `src/ClassicUO.Client/Game/Managers/ClassicUOInjectionApiBridge.cs`; funzione `GetFindDistance`.

#### 3. SetFindDistance

Il bridge legge l’impostazione runtime oppure limita e memorizza l’intero, senza esaminare il mondo.

Integer facoltativo. Senza argomento legge; value imposta. Limitato a 0..255; un negativo diventa 0, non illimitato. Nuovo runtime: 18; lo stato ripristinato può differire. I decimali sono troncati verso zero; sono ammesse stringhe numeriche decimal/0x. Usare Integer evita conversioni implicite.

Sorgente del progetto: `src/ClassicUO.Client/Game/Managers/ClassicUOInjectionApiBridge.cs`; funzione `SetFindDistance`.
...
# Legge o modifica il raggio orizzontale predefinito della ricerca.
#
# Senza argomenti: Integer, limite corrente (distanza in caselle), non ID, conteggio o Boolean.
# 0 indica limite zero, non fallimento. Con value: Unit, nessun valore, né TRUE/FALSE né valore
# precedente. Rileggere FindDistance() dopo la modifica.

SUB Main()
    # previous salva l’impostazione reale. value:=5 imposta un limite normale; 1000 viene ridotto a
    # 255. Print legge separatamente. Finally ripristina previous.
...
- previous salva l’impostazione reale. value:=5 imposta un limite normale; 1000 viene ridotto a 255. Print legge separatamente. Finally ripristina previous.
...
SUB Main()
    # previous conserva il valore del chiamante. 5 cambia solo FindDistance; l’altro limite resta.
...
- previous conserva il valore del chiamante. 5 cambia solo FindDistance; l’altro limite resta. 0x0EED: grafica dell’oro; -1: qualsiasi colore; Container=-1: mondo; FALSE: nessuna ricorsione dei contenitori. id è un serial; <> 0 verifica la presenza. FindCount conta oggetti/pile. Finally ripristina il limite, non la lista.
