...
Скасовує вибраний обмін і закриває його вікно.
...
Integer: 1 = TRUE — вікно знайдено, закрито та через закриття надіслано скасування; 0 = FALSE — вікна немає. Не чекає відповіді сервера. Повторне закриття не надсилає другий пакет.
...
Код проєкту: `src/ClassicUO.Client/Game/Managers/ClassicUOInjectionApiBridge.cs`; функція `CancelTrade`.
...
Dispose одразу завершується для вже закритого вікна, звільняє UI й один раз викликає GameActions.CancelTrade(ID1).

Integer: 1 = TRUE — вікно знайдено, закрито та через закриття надіслано скасування; 0 = FALSE — вікна немає. Не чекає відповіді сервера. Повторне закриття не надсилає другий пакет.

Код проєкту: `src/ClassicUO.Client/Game/UI/Gumps/TradingGump.cs`; функція `Dispose`.
...
# Скасовує вибраний обмін і закриває його вікно.
#
# Integer: 1 = TRUE — вікно знайдено, закрито та через закриття надіслано скасування; 0 = FALSE
# — вікна немає. Не чекає відповіді сервера. Повторне закриття не надсилає другий пакет.
#
