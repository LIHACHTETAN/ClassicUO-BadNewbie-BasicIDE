...
- Події, оголошені скриптом, як і раніше потребують явного RaiseEvent. Автоматичні підписки на журнал, ресурси та підключення використовують події клієнта UO. з Basic.GameEvents. Handles, WithEvents, Custom Event, типи делегатів подій і події класів не підтримуються.
