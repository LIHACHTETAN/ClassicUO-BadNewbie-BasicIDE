...
Читає або змінює типовий горизонтальний радіус пошуку.
...
- `value` — Необов’язковий Integer. Без аргументу — читання; value — встановлення. Межі 0..255; від’ємне значення стає 0, а не безлімітом. Новий runtime: 18; відновлений стан може мати інше значення. Дробова частина відкидається до нуля; числові рядки decimal/0x теж приймаються. Використовуйте Integer без неявних перетворень.

## Повертає

Без аргументів: Integer — поточна межа (відстань у клітинках), не ID, кількість об’єктів чи Boolean. 0 — нульова межа, не невдача. З value: Unit — значення немає, це не TRUE/FALSE і не попередня настройка. Прочитайте FindDistance() після запису.

## Поведінка

- Відстань: max(abs(dx), abs(dy)) від поточної точки відліку клієнта; діагональ рахується за одну клітинку. Межу включено. 0 допускає лише ті самі X/Y. Для рухомого Mobile враховується кінцева позиція черги кроків.
...
- Джерело: [Stealth FindDistance](https://stealth.od.ua/api/FindDistance/). Цей клієнт має власні початкові значення та межі: FindDistance 18 / 0..255; FindVertical 2 / 0..120. Наведені синтаксис Basic і розширені фільтри стосуються цього проєкту.
...
Код проєкту: `src/ClassicUO.Client/Game/Managers/ClassicUOInjectionApiBridge.cs`; функція `GetFindDistance`.

#### 3. SetFindDistance

Bridge читає настройку runtime або обмежує й записує ціле число. Світ тут не сканується.

Необов’язковий Integer. Без аргументу — читання; value — встановлення. Межі 0..255; від’ємне значення стає 0, а не безлімітом. Новий runtime: 18; відновлений стан може мати інше значення. Дробова частина відкидається до нуля; числові рядки decimal/0x теж приймаються. Використовуйте Integer без неявних перетворень.

Код проєкту: `src/ClassicUO.Client/Game/Managers/ClassicUOInjectionApiBridge.cs`; функція `SetFindDistance`.
...
# Читає або змінює типовий горизонтальний радіус пошуку.
#
# Без аргументів: Integer — поточна межа (відстань у клітинках), не ID, кількість об’єктів чи
# Boolean. 0 — нульова межа, не невдача. З value: Unit — значення немає, це не TRUE/FALSE і не
# попередня настройка. Прочитайте FindDistance() після запису.

SUB Main()
    # previous зберігає фактичне значення. value:=5 задає звичайну межу; 1000 обмежується до 255.
...
- previous зберігає фактичне значення. value:=5 задає звичайну межу; 1000 обмежується до 255. Print читає результат окремим викликом. Finally відновлює previous.
...
SUB Main()
    # previous зберігає настройку викликача. 5 змінює тільки FindDistance; друга межа незмінна.
...
- previous зберігає настройку викликача. 5 змінює тільки FindDistance; друга межа незмінна. 0x0EED — графіка золота, -1 — будь-який hue, Container=-1 — світ, FALSE — без рекурсії контейнерів. id — serial; <> 0 перевіряє наявність. FindCount рахує об’єкти/стоси. Finally відновлює настройку, не список.
