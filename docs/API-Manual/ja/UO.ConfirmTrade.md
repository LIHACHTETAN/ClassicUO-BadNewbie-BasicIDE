...
Integer：ウィンドウが存在し、自分の同意が設定済みまたは設定された場合は 1 = TRUE、なければ 0 = FALSE。サーバーでの完了を証明しません。繰り返しても解除しません。
...
プロジェクトのソース: `src/ClassicUO.Client/Game/Managers/ClassicUOInjectionApiBridge.cs`; 関数 `ConfirmTrade`.
