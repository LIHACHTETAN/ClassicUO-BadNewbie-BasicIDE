...
- `value` — 省略可能な Integer。省略すると読み取り、value を指定すると設定します。0..120 に制限され、負数は無制限ではなく 0 になります。新規 runtime の既定値は 2。復元した状態では異なる場合があります。小数はゼロ方向へ切り捨て、decimal/0x の数値文字列も受け付けます。暗黙変換を避けるには Integer を使用してください。

## 戻り値

引数なし：Integer、現在の制限（ワールド Z 単位の高低差）。ID、個数、Boolean ではありません。0 は失敗ではなく範囲ゼロです。value あり：Unit、戻り値なし。TRUE/FALSE や変更前の値ではありません。設定後に FindVertical() で保存値を確認できます。

## 動作

- 高さは abs(object.Z - player.Z)。上下両方向で境界を含みます。0 は同じ Z のみ許可します。階番号や水平タイル数ではありません。
...
- 参照：[Stealth FindVertical](https://stealth.od.ua/api/FindVertical/)。本クライアント固有の初期値／範囲は FindDistance 18 / 0..255、FindVertical 2 / 0..120 です。上記の Basic 構文と拡張フィルターは本プロジェクトの仕様です。
...
プロジェクトのソース: `src/ClassicUO.Client/Game/Managers/ClassicUOInjectionApiBridge.cs`; 関数 `GetFindVertical`.

#### 3. SetFindVertical

Bridge は runtime の設定を読むか、整数を制限して保存します。ワールドは走査しません。

省略可能な Integer。省略すると読み取り、value を指定すると設定します。0..120 に制限され、負数は無制限ではなく 0 になります。新規 runtime の既定値は 2。復元した状態では異なる場合があります。小数はゼロ方向へ切り捨て、decimal/0x の数値文字列も受け付けます。暗黙変換を避けるには Integer を使用してください。

プロジェクトのソース: `src/ClassicUO.Client/Game/Managers/ClassicUOInjectionApiBridge.cs`; 関数 `SetFindVertical`.
...
# 検索時に許容する既定の高低差を読み取るか変更します。
#
# 引数なし：Integer、現在の制限（ワールド Z 単位の高低差）。ID、個数、Boolean ではありません。0 は失敗ではなく範囲ゼロです。value
# あり：Unit、戻り値なし。TRUE/FALSE や変更前の値ではありません。設定後に FindVertical() で保存値を確認できます。

SUB Main()
    # previous に実際の設定を保存します。value:=10 は通常の制限、1000 は 120 への制限を示します。Print は別の読み取りで結果を表示します。Finally で
    # previous を復元します。
...
- previous に実際の設定を保存します。value:=10 は通常の制限、1000 は 120 への制限を示します。Print は別の読み取りで結果を表示します。Finally で previous を復元します。
...
SUB Main()
    # previous は呼び出し元の設定です。10 は FindVertical のみ変更し、もう一方は保持します。0x0EED は金貨のグラフィック、-1 は全色、Container=-1
...
- previous は呼び出し元の設定です。10 は FindVertical のみ変更し、もう一方は保持します。0x0EED は金貨のグラフィック、-1 は全色、Container=-1 はワールド、FALSE はコンテナ再帰なしです。id は serial、<> 0 は存在確認です。FindCount はオブジェクト／スタック数です。Finally は設定を戻し、結果リストは戻しません。
