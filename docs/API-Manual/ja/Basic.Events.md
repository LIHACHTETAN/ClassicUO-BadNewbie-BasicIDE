...
- スクリプト宣言イベントには引き続き RaiseEvent が必要です。ジャーナル・資源・接続の自動購読は Basic.GameEvents に記載したクライアント UO. イベントを使用します。Handles、WithEvents、Custom Event、イベントデリゲート型、クラスイベントは未対応です。
