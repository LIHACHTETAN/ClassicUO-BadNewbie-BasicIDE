...
- `value` — 선택적 Integer입니다. 인수가 없으면 읽고, value가 있으면 설정합니다. 0..255 범위로 제한하며 음수는 무제한이 아닌 0입니다. 새 runtime 기본값은 18이고 복원한 상태는 다를 수 있습니다. 소수는 0 방향으로 잘라 내며 decimal/0x 숫자 문자열도 허용합니다. 암시적 변환을 피하려면 Integer를 사용하세요.

## 반환값

인수 없음: Integer, 현재 제한(타일 단위 거리)이며 ID, 개수, Boolean이 아닙니다. 0은 실패가 아니라 범위 0입니다. value 있음: Unit, 반환값이 없으며 TRUE/FALSE나 이전 값도 아닙니다. 설정 후 FindDistance()으로 저장값을 읽으세요.

## 동작

- 거리는 클라이언트의 현재 거리 기준점에서 max(abs(dx), abs(dy))로 계산합니다. 대각선도 한 타일이며 경계를 포함합니다. 0은 같은 X/Y만 허용합니다. 이동 중인 Mobile은 걸음 대기열의 최종 위치를 사용합니다.
...
- 참고: [Stealth FindDistance](https://stealth.od.ua/api/FindDistance/). 이 클라이언트의 고유 기본값/범위는 FindDistance 18 / 0..255, FindVertical 2 / 0..120입니다. 위 Basic 문법과 확장 필터는 이 프로젝트를 설명합니다.
...
프로젝트 소스: `src/ClassicUO.Client/Game/Managers/ClassicUOInjectionApiBridge.cs`; 함수 `GetFindDistance`.

#### 3. SetFindDistance

Bridge는 runtime 설정을 읽거나 정수를 제한하여 저장합니다. 월드를 순회하지 않습니다.

선택적 Integer입니다. 인수가 없으면 읽고, value가 있으면 설정합니다. 0..255 범위로 제한하며 음수는 무제한이 아닌 0입니다. 새 runtime 기본값은 18이고 복원한 상태는 다를 수 있습니다. 소수는 0 방향으로 잘라 내며 decimal/0x 숫자 문자열도 허용합니다. 암시적 변환을 피하려면 Integer를 사용하세요.

프로젝트 소스: `src/ClassicUO.Client/Game/Managers/ClassicUOInjectionApiBridge.cs`; 함수 `SetFindDistance`.
...
# 기본 수평 검색 반경을 읽거나 변경합니다.
#
# 인수 없음: Integer, 현재 제한(타일 단위 거리)이며 ID, 개수, Boolean이 아닙니다. 0은 실패가 아니라 범위 0입니다. value 있음: Unit,
# 반환값이 없으며 TRUE/FALSE나 이전 값도 아닙니다. 설정 후 FindDistance()으로 저장값을 읽으세요.

SUB Main()
    # previous는 실제 설정을 저장합니다. value:=5은 일반 제한이며 1000은 255으로 제한됩니다. Print는 별도 읽기로 결과를 출력합니다. Finally는
    # previous를 복원합니다.
...
- previous는 실제 설정을 저장합니다. value:=5은 일반 제한이며 1000은 255으로 제한됩니다. Print는 별도 읽기로 결과를 출력합니다. Finally는 previous를 복원합니다.
...
SUB Main()
    # previous는 호출자의 설정입니다. 5은 FindDistance만 변경하고 다른 제한은 유지합니다. 0x0EED는 금 그래픽, -1은 모든 색,
...
- previous는 호출자의 설정입니다. 5은 FindDistance만 변경하고 다른 제한은 유지합니다. 0x0EED는 금 그래픽, -1은 모든 색, Container=-1은 월드, FALSE는 컨테이너 재귀 없음입니다. id는 serial이며 <> 0은 존재 여부 검사입니다. FindCount는 객체/스택 개수입니다. Finally는 설정만 복원하고 검색 목록은 복원하지 않습니다.
