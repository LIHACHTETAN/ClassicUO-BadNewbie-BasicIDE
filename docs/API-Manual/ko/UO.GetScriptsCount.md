...
Integer >= 0: 일시 정지된 실행을 포함한 활성 실행 수입니다. Boolean이나 인덱스가 아닙니다.
...
- 인수가 없습니다. 개수를 세면서 목록을 정렬하거나 실행 상태를 바꾸지 않습니다.
...
프로젝트 소스: `src/ClassicUO.Client/Game/Managers/ClassicUOInjectionApiBridge.cs`; 함수 `GetScriptsCount`.
...
프로젝트 소스: `src/ClassicUO.Client/Game/Managers/YokoInjectionManager.cs`; 함수 `GetScriptsCount`.
...
SUB Main()
    ...
    # HasOtherScripts는 호출자가 보통 한 항목을 차지하므로 1과 비교합니다. GetScriptsCount와 달리 보조 함수만 true/false를 반환합니다.
    # Integer >= 0: 일시 정지된 실행을 포함한 활성 실행 수입니다. Boolean이나 인덱스가 아닙니다.
