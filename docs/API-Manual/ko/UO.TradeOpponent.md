...
상대 컨테이너 serial을 16진수 문자열로 반환하며 캐릭터 ID가 아닙니다.
...
String: 상대 컨테이너 ID2의 16진수 문자열이며 없으면 "0x00000000"입니다. 캐릭터 ID는 GetTradeOpponent(index + 1)을 사용합니다.
...
프로젝트 소스: `external/InjectionScript/src/InjectionScript/Runtime/InjectionApiUO.cs`; 함수 `TradeOpponent`.
...
프로젝트 소스: `src/ClassicUO.Client/Game/Managers/ClassicUOInjectionApiBridge.cs`; 함수 `TradeOpponent`.
...
# 상대 컨테이너 serial을 16진수 문자열로 반환하며 캐릭터 ID가 아닙니다.
#
# String: 상대 컨테이너 ID2의 16진수 문자열이며 없으면 "0x00000000"입니다. 캐릭터 ID는 GetTradeOpponent(index + 1)을
# 사용합니다.
