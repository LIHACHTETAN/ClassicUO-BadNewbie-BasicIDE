...
String: 저장된 소스 파일 경로이며, 인덱스가 없으면 ""입니다. 파일 시작은 보통 전체 경로를 가지지만 명령이나 메모리의 소스에는 일반 파일이 없을 수 있습니다.
...
- ScriptIndex=0은 현재 첫 실행을 선택합니다. 소스 경로를 읽을 뿐 파일을 열거나 저장하거나 실행하지 않습니다.
...
프로젝트 소스: `src/ClassicUO.Client/Game/Managers/ClassicUOInjectionApiBridge.cs`; 함수 `GetScriptPath`.
...
프로젝트 소스: `src/ClassicUO.Client/Game/Managers/YokoInjectionManager.cs`; 함수 `GetScriptPath`.
...
SUB Main()
    ...
    # ReadScriptPath는 경로가 비었을 때만 fallback을 사용합니다. GetScriptPath의 새 오버로드가 아닙니다.
    # String: 저장된 소스 파일 경로이며, 인덱스가 없으면 ""입니다. 파일 시작은 보통 전체 경로를 가지지만 명령이나 메모리의 소스에는 일반 파일이 없을 수 있습니다.
    # 필수 ScriptIndex는 최신 GetScriptsList에서 얻는 0부터 시작하는 정수 인덱스입니다. 음수이거나 없는 인덱스는 명시된 빈 값/알 수 없음 결과를
    # 반환합니다. 아이템 serial, 프로시저 이름 또는 IDE 실행 ID를 전달하지 마세요.
