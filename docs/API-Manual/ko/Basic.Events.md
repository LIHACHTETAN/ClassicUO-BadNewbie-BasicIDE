...
- 스크립트 선언 이벤트는 여전히 명시적인 RaiseEvent가 필요합니다. 저널·자원·연결 자동 구독은 Basic.GameEvents의 클라이언트 UO. 이벤트를 사용합니다. Handles, WithEvents, Custom Event, 이벤트 대리자 형식 및 클래스 이벤트는 지원하지 않습니다.
