...
JsonNull 객체로 null로 저장됩니다.0이나 빈 String이 아닙니다. JsonKind(value)="null"로 확인합니다.
...
프로젝트 소스: `external/InjectionScript/src/InjectionScript/Runtime/ObjectTypes/JsonPrimitiveObjects.cs`; 함수 `JsonNullObject`.
...
Option Explicit On
Sub Main()
    # Main을 실행합니다. JsonNull() → Object; JsonStringify → String "null".
...
Option Explicit On
Sub Main()
    # Main을 실행합니다. JsonNull() → d["selected"]; JsonKind comparison → Integer1/True.
...
Option Explicit On
Sub Main()
    # Main을 실행합니다. JsonNull(),0,"" → three distinct values → String [null,0,""] .
