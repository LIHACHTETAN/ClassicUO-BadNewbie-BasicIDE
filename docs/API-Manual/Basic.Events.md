...
- События, объявленные скриптом, по-прежнему требуют явного RaiseEvent. Автоматические подписки на журнал, ресурсы и подключение используют события клиента UO., описанные в Basic.GameEvents. Handles, WithEvents, Custom Event, типы делегатов событий и события классов не поддерживаются.
