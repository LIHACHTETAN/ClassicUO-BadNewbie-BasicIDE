...
- `container / Container` — В Count(type,color,container): my или числовой -1 — все предметы, корневой владелец которых self (включая экипировку и вложенные сумки); backpack/serial — содержимое указанного контейнера рекурсивно; UO.Ground(), явные 0/"0", ground или 1 — предметы и Mobile на земле. В CountEx числовой -1 и 0xFFFFFFFF тоже означают землю, а конкретный serial — только прямое содержимое. CountEx принимает backpack и имена AddObject; неизвестное имя вызывает ошибку скрипта. Проверяйте полученный serial до вызова: явный ноль выбирает землю, а не отсутствующий контейнер.
...
- В этом Basic CountEx заменяет общий результат поиска так же, как Count. Это отличается от справки Stealth CountEx, где поля поиска восстанавливаются. Сохраните UO.GetFoundItems() до подсчёта, если прежний массив ID ещё нужен.
