...
- После As также поддерживается имя объявленной Structure. VAR и DIM с этим типом без инициализатора создают поля по умолчанию. Принимается именно этот объявленный тип структуры; разные структурные типы друг в друга не преобразуются. Копирование вложенных значений и ссылочные поля описаны в Structure / New.
