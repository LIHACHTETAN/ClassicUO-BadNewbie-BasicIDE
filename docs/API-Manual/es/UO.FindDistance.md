...
Lee o cambia el radio horizontal de búsqueda predeterminado.
...
- `value` — Integer opcional. Sin argumento lee; value establece. Se limita a 0..255; un negativo pasa a 0, no a ilimitado. Runtime nuevo: 18; un estado restaurado puede diferir. Los decimales se truncan hacia cero; también admite cadenas numéricas decimal/0x. Use Integer para evitar conversiones implícitas.

## Devuelve

Sin argumentos: Integer, límite actual (distancia en casillas), no un ID, cantidad de objetos o Boolean. 0 significa límite cero, no fallo. Con value: Unit, sin valor, ni TRUE/FALSE ni valor anterior. Lea FindDistance() después de escribir.

## Comportamiento

- Distancia: max(abs(dx), abs(dy)) desde el origen actual de alcance del cliente; una diagonal cuenta como una casilla. Incluye el límite; 0 exige las mismas X/Y. En Mobiles en movimiento se usa la posición final de la cola de pasos.
...
- Referencia: [Stealth FindDistance](https://stealth.od.ua/api/FindDistance/). Este cliente conserva valores iniciales/rangos propios: FindDistance 18 / 0..255; FindVertical 2 / 0..120. La sintaxis Basic y los filtros ampliados describen este proyecto.
...
Código del proyecto: `src/ClassicUO.Client/Game/Managers/ClassicUOInjectionApiBridge.cs`; función `GetFindDistance`.

#### 3. SetFindDistance

El bridge lee el ajuste runtime o limita y guarda el entero, sin recorrer el mundo.

Integer opcional. Sin argumento lee; value establece. Se limita a 0..255; un negativo pasa a 0, no a ilimitado. Runtime nuevo: 18; un estado restaurado puede diferir. Los decimales se truncan hacia cero; también admite cadenas numéricas decimal/0x. Use Integer para evitar conversiones implícitas.

Código del proyecto: `src/ClassicUO.Client/Game/Managers/ClassicUOInjectionApiBridge.cs`; función `SetFindDistance`.
...
# Lee o cambia el radio horizontal de búsqueda predeterminado.
#
# Sin argumentos: Integer, límite actual (distancia en casillas), no un ID, cantidad de objetos
# o Boolean. 0 significa límite cero, no fallo. Con value: Unit, sin valor, ni TRUE/FALSE ni
# valor anterior. Lea FindDistance() después de escribir.

SUB Main()
    # previous guarda el ajuste real. value:=5 fija un límite normal; 1000 se reduce a 255. Print
...
- previous guarda el ajuste real. value:=5 fija un límite normal; 1000 se reduce a 255. Print lee por separado. Finally restaura previous.
...
SUB Main()
    # previous conserva el valor del llamador. 5 cambia solo FindDistance; el otro límite permanece.
    # 0x0EED: gráfico del oro; -1: cualquier color; Container=-1: mundo; FALSE: sin recursión de
    # contenedores. id es un serial; <> 0 comprueba existencia. FindCount cuenta objetos/pilas.
    # Finally restaura el ajuste, no la lista.
...
- previous conserva el valor del llamador. 5 cambia solo FindDistance; el otro límite permanece. 0x0EED: gráfico del oro; -1: cualquier color; Container=-1: mundo; FALSE: sin recursión de contenedores. id es un serial; <> 0 comprueba existencia. FindCount cuenta objetos/pilas. Finally restaura el ajuste, no la lista.
