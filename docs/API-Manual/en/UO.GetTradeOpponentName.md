...
Returns the partner name received with the trade window.
...
String: name from the trade-opening packet; empty if no window/name. Does not request a fresh name.
...
Project source: `src/ClassicUO.Client/Game/Managers/ClassicUOInjectionApiBridge.cs`; function `GetTradeOpponentName`.
...
# Returns the partner name received with the trade window.
#
# String: name from the trade-opening packet; empty if no window/name. Does not request a fresh
# name.
