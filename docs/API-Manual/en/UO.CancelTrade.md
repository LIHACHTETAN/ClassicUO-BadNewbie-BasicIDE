...
Cancels the selected trade and closes its window.
...
Integer: 1 = TRUE if the window was found, closed and its cancellation sent through disposal; 0 = FALSE if absent. Does not wait for a server reply. Repeated disposal sends no second packet.
...
Project source: `src/ClassicUO.Client/Game/Managers/ClassicUOInjectionApiBridge.cs`; function `CancelTrade`.
...
Dispose returns immediately for an already disposed window, releases UI and calls GameActions.CancelTrade(ID1) once.

Integer: 1 = TRUE if the window was found, closed and its cancellation sent through disposal; 0 = FALSE if absent. Does not wait for a server reply. Repeated disposal sends no second packet.

Project source: `src/ClassicUO.Client/Game/UI/Gumps/TradingGump.cs`; function `Dispose`.
...
# Cancels the selected trade and closes its window.
#
# Integer: 1 = TRUE if the window was found, closed and its cancellation sent through disposal;
# 0 = FALSE if absent. Does not wait for a server reply. Repeated disposal sends no second
# packet.
#
