...
String：例如 "0x40000011" 的十六進位 ID；不存在時為 "0x00000000"。這是自己的容器 ID，不是 Boolean 或空字串。
...
專案原始碼: `external/InjectionScript/src/InjectionScript/Runtime/InjectionApiUO.cs`; 函式 `TradeContainer`.
...
專案原始碼: `src/ClassicUO.Client/Game/Managers/ClassicUOInjectionApiBridge.cs`; 函式 `TradeContainer`.
