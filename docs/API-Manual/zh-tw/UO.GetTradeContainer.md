...
- `Num` — 1 是自己的容器，2 是對方容器；其他值傳回 0。

## 傳回值

Integer：容器 serial 的 32 位元；視窗不存在或 Num 無效時為 0。不是圖形 type。保留最高位元：用 <> 0，不要用 > 0 或 = TRUE。
...
專案原始碼: `src/ClassicUO.Client/Game/Managers/ClassicUOInjectionApiBridge.cs`; 函式 `GetTradeContainer`.
