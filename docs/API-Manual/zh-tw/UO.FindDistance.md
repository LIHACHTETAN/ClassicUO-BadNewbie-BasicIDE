...
- `value` — 選用的 Integer。省略參數時讀取；value 用來設定。儲存範圍為 0..255；負數變成 0，不代表無限。新 runtime 的預設值為 18；還原的狀態可能不同。小數朝零截斷，也接受 decimal/0x 數字字串。建議使用 Integer，避免隱含轉換。

## 傳回值

無參數：Integer，目前的限制（以格數表示的距離），不是物件 ID、數量或 Boolean。0 代表零範圍，不代表失敗。有 value：Unit，沒有回傳值，也不是 TRUE/FALSE 或舊設定。設定後可呼叫 FindDistance() 讀取儲存值。

## 行為

- 距離為從客戶端目前距離基準點計算的 max(abs(dx), abs(dy))；斜向一格仍算一格。包含邊界；0 只接受相同 X/Y。移動中的 Mobile 使用排程步驟的最終位置計算距離。
...
- 參考：[Stealth FindDistance](https://stealth.od.ua/api/FindDistance/)。此客戶端保留自己的初始值與範圍：FindDistance 18 / 0..255；FindVertical 2 / 0..120。以上 Basic 語法與延伸篩選描述本專案。
...
專案原始碼: `src/ClassicUO.Client/Game/Managers/ClassicUOInjectionApiBridge.cs`; 函式 `GetFindDistance`.

#### 3. SetFindDistance

Bridge 讀取 runtime 設定，或限制並儲存整數，不掃描世界。

選用的 Integer。省略參數時讀取；value 用來設定。儲存範圍為 0..255；負數變成 0，不代表無限。新 runtime 的預設值為 18；還原的狀態可能不同。小數朝零截斷，也接受 decimal/0x 數字字串。建議使用 Integer，避免隱含轉換。

專案原始碼: `src/ClassicUO.Client/Game/Managers/ClassicUOInjectionApiBridge.cs`; 函式 `SetFindDistance`.
...
# 讀取或設定預設的水平搜尋半徑。
#
# 無參數：Integer，目前的限制（以格數表示的距離），不是物件 ID、數量或 Boolean。0 代表零範圍，不代表失敗。有 value：Unit，沒有回傳值，也不是
# TRUE/FALSE 或舊設定。設定後可呼叫 FindDistance() 讀取儲存值。

SUB Main()
    # previous 儲存實際設定。value:=5 設定一般限制；1000 示範被限制為 255。Print 透過另一次讀取輸出結果。Finally 還原 previous。
...
SUB Main()
    # previous 保留呼叫端設定。5 只改變 FindDistance，另一限制不變。0x0EED 是金幣圖形，-1 為任何色彩，Container=-1 為世界，FALSE
...
- previous 保留呼叫端設定。5 只改變 FindDistance，另一限制不變。0x0EED 是金幣圖形，-1 為任何色彩，Container=-1 為世界，FALSE 停用容器遞迴。id 是 serial；<> 0 檢查是否找到。FindCount 計算完整物件／堆疊。Finally 還原設定，不還原搜尋清單。
