...
String：對方容器 ID2 的十六進位字串；不存在時為 "0x00000000"。角色 ID 請用 GetTradeOpponent(index + 1)。
...
專案原始碼: `external/InjectionScript/src/InjectionScript/Runtime/InjectionApiUO.cs`; 函式 `TradeOpponent`.
...
專案原始碼: `src/ClassicUO.Client/Game/Managers/ClassicUOInjectionApiBridge.cs`; 函式 `TradeOpponent`.
