...
String：保存的對方名稱；缺少視窗或名稱時為空字串。不會讀取容器物品的名稱。
...
專案原始碼: `external/InjectionScript/src/InjectionScript/Runtime/InjectionApiUO.cs`; 函式 `TradeName`.
...
專案原始碼: `src/ClassicUO.Client/Game/Managers/ClassicUOInjectionApiBridge.cs`; 函式 `TradeName`.
